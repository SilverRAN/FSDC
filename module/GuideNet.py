"""https://github.com/kakaxi314/GuideNet.git"""
...
class GuideNet(nn.Module):
    ...
    def forward(self, img, lidar):
        c0_img = self.conv_img(img) # c0: [32, H, W]
        c1_img = self.layer1_img(c0_img) # c1: [64, H/2, W/2]
        c2_img = self.layer2_img(c1_img) # c2: [128, H/4, W/4]
        c3_img = self.layer3_img(c2_img) # c3: [256, H/8, W/8]
        c4_img = self.layer4_img(c3_img) # c4: [256, H/16, W/16]
        c5_img = self.layer5_img(c4_img) # c5: [256, H/32, W/32]
        dc5_img = self.layer5d_img(c5_img) # dc5: [256, H/16, W/16]
        c4_mix = dc5_img + c4_img
        dc4_img = self.layer4d_img(c4_mix) # dc4: [256, H/8, W/8]
        c3_mix = dc4_img + c3_img
        dc3_img = self.layer3d_img(c3_mix) # dc3: [128, H/4, W/4]
        c2_mix = dc3_img + c2_img
        dc2_img = self.layer2d_img(c2_mix) # dc2: [64, H/2, W/2]
        c1_mix = dc2_img + c1_img

        c0_lidar = self.conv_lidar(lidar)
        c1_lidar = self.layer1_lidar(c0_lidar) # 1/2
        c1_lidar_dyn = self.guide1(c1_lidar, c1_mix)
        c2_lidar = self.layer2_lidar(c1_lidar_dyn) # 1/4
        c2_lidar_dyn = self.guide2(c2_lidar, c2_mix)
        c3_lidar = self.layer3_lidar(c2_lidar_dyn) # 1/8
        c3_lidar_dyn = self.guide3(c3_lidar, c3_mix)
        c4_lidar = self.layer4_lidar(c3_lidar_dyn) # 1/16
