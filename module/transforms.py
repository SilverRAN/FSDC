...
class RandomCenterCrop(object):
    """Crops the given ``numpy.ndarray`` at the center.
    Args:
        center (x, y): Given crop center. 
        out_size (h, w): Ouput size of image.
    """
    def __init__(self, center, out_size):
        self.center = center
    ...
    def __call__(self, img):
        ...
        y_t = self.center[0] + (self.out_size[0] // 2)
        y_b = self.center[0] - (self.out_size[0] // 2)
        x_l = self.center[1] - (self.out_size[1] // 2)
        x_r = self.center[1] + (self.out_size[1] // 2)
        assert x_l >= 0 and x_l <= w
        assert x_r >= 0 and x_r <= w
        assert y_t >= 0 and y_t <= h
        assert y_b >= 0 and y_b <= h
